...
<div class="w-full flex justify-end mb-4">
    <button class="bg-gray-800 text-xl text-white px-2 py-1 rounded-md border border-white" onclick={() => marcaModel.showCreateModal()}>
        Nueva Marca
    </button>
</div>
...
<table class="flex-1 w-full">
    ...
    <tbody>
        {#each marcaModel.marcas as marca}
            <tr class="odd:bg-gray-100 dark:odd:bg-gray-700">
                <td class="px-2 py-1">{marca.name}</td>
                <td class="px-2 py-1">{marca.description}</td>
                <td class="px-2 py-1">
                    <div class="flex justify-center gap-2">
                        <button
                            onclick={() => marcaModel.showEditModal(marca)}
                            aria-label="Editar"
                        ...
                        <button
                            onclick={() => marcaModel.showDeleteModal(marca)}
                            aria-label="Eliminar"
