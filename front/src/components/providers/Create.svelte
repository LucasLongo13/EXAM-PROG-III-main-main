...
<div class="w-full h-full fixed top-0 left-0 flex items-center justify-center bg-black/40 dark:bg-black/60 backdrop-blur-sm z-50">
    <div class="bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 p-4 rounded-md w-96 shadow-lg">
        <form onsubmit={(e) => providerModel.createSupplier(e)}>
            <h2 class="text-lg font-bold">Crear Proveedor</h2>
            <hr />

            <div class="p-2 flex flex-col">
                <label for="name" class="font-bold">Nombre:</label>
                <input id="name" class="border p-2 rounded-md bg-white text-slate-900 dark:bg-slate-700 dark:text-slate-100 placeholder:text-slate-400" name="name" placeholder="Nombre del proveedor" required />
            ...
            <div class="p-2 flex flex-col">
                ...
                <input id="email" type="email" class="border p-2 rounded-md bg-white text-slate-900 dark:bg-slate-700 dark:text-slate-100 placeholder:text-slate-400" name="email" placeholder="Correo electrónico" />
            ...
            <div class="p-2 flex flex-col">
                ...
                <input id="phone" type="tel" inputmode="numeric" pattern="[0-9]*" class="border p-2 rounded-md bg-white text-slate-900 dark:bg-slate-700 dark:text-slate-100 placeholder:text-slate-400" name="phone" placeholder="Teléfono" oninput={(e) => (e as Event & { currentTarget: HTMLInputElement }).currentTarget.value = String((e as Event & { currentTarget: HTMLInputElement }).currentTarget.value).replace(/\D/g, '')} />
